...
import { FINISH_TRANSITION, IGNORED_TRANSITION, START_TRANSITION } from '@uirouter/redux/lib-esm/core/actions';
import { Transition } from '@uirouter/react';

import { RouterState, RouterActiveState, TransitionAction, RouteTarget } from './interfaces';
 ...
 */
export const routerReducer = (state = initialState, action: TransitionAction) => {
    const { type } = action;
    ...
    switch (type) {
        ...
        case IGNORED_TRANSITION:
            return state.withMutations((currentState: RouterState) => {
                currentState.set('transitioning', false).set('activeState', getActiveState(transition));
            });
        default:
            return state;
    }
